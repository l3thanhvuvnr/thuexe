...
?>
<script type="text/html" id="tmpl-iee_image_compare-shortcode">
<# if ( '' !== after_image || '' !== before_image ) { #>
<div {{{ _.fusionGetAttributes( attr ) }}}>
	<figure class="elegant-image-compare-container">
			<img src="{{{ after_image }}}">
			<span {{{ _.fusionGetAttributes( afterImageAttr ) }}}>{{{ after_image_caption }}}</span>
			<div class="elegant-image-compare-after-image">
				<img src="{{{ before_image }}}">
				<span {{{ _.fusionGetAttributes( beforeImageAttr ) }}}>{{{ before_image_caption }}}</span>
			</div>
			<span {{{ _.fusionGetAttributes( dragHandleAttr ) }}}></span>
	</figure>
</div>
<# } else { #>
