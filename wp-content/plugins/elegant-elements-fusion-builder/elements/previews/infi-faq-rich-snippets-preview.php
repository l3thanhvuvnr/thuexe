<script type="text/template" id="elegant-elements-module-infi-faq-rich-snippets-preview-template">
	<h4 class="fusion_module_title"><span class="fusion-module-icon {{ fusionAllElements[element_type].icon }}"></span>{{ fusionAllElements[element_type].name }}</h4>
	<ul>
		<#
		var
		content = typeof params.element_content !== 'undefined' ? params.element_content : '',
		shortcode_reg_exp = window.wp.shortcode.regexp( 'iee_faq_rich_snippet_item' ),
		shortcode_inner_reg_exp = new RegExp( '\\[(\\[?)(iee_faq_rich_snippet_item)(?![\\w-])([^\\]\\/]*(?:\\/(?!\\])[^\\]\\/]*)*?)(?:(\\/)\\]|\\](?:([^\\[]*(?:\\[(?!\\/\\2\\])[^\\[]*)*)(\\[\\/\\2\\]))?)(\\]?)' ),
		...
		_.each( shortcode_matches, function ( inner_item ) {
			...
			if ( counter < 3 ) {
				...
				#>
				<li>{{ shortcode_attributes.named['question'] }}</li>
				<#
			}
