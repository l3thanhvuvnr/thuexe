 ...
 * Envato API class.
 *
 * @package Elegant_Elements_Updater
 */
 ...
 *
 * @class Elegant_Elements_Updater
 * @version 1.0.0
 * @since 1.0.0
 */
final class Elegant_Elements_Updater {

	/**
	 * The arguments that are used in the Elegant_Product_Registration class.
	 *
	 * @access private
	 * @since 1.0.0
	 * @var array
	...
	/**
	 * An instance of the Elegant_Product_Registration class.
	 *
	 * @access private
	 * @since 1.0.0
	 ...
	 *
	 * @access public
	 * @param object $registration An instance of the Elegant_Product_Registration class.
	 */
	...
	public function deferred_download( $id ) {
		...
		return add_query_arg( $args, esc_url( admin_url( 'admin.php?page=elegant-elements-options' ) ) );
	}
	...
	/**
	 * Inject update data for elegant elements.
	 *
	 * @since 1.0.0
	...
	public function update_plugins( $transient ) {
		...
		// If product is not registtered, there won't be any plugins available.
		if ( empty( $plugins ) ) {
			return $transient;
		}

		// Set plugin file name.
		$plugin_file = 'elegant-elements-fusion-builder/elegant-elements-fusion-builder.php';
		$plugin_data = get_plugin_data( ELEGANT_ELEMENTS_PLUGIN_FILE );

		// Loop through all the plugins and process elegant element plugin updates.
		foreach ( $plugins as $key => $plugin ) {
			if ( 'Elegant Elements for Fusion Builder' === $plugin['name'] ) {
				if ( version_compare( ELEGANT_ELEMENTS_VERSION, $plugin['version'], '<' ) ) {
					$_plugin                             = array(
						...
						'icons'       => array(
							'1x' => esc_url_raw( ELEGANT_ELEMENTS_PLUGIN_URL . '/assets/admin/img/icon.png' ),
							'2x' => esc_url_raw( ELEGANT_ELEMENTS_PLUGIN_URL . '/assets/admin/img/icon.png' ),
						),
					);
	...
	public function plugins_api( $response, $action, $args ) {
		...
		if ( 'plugin_information' === $action && isset( $args->slug ) ) {
			...
			foreach ( $installed as $slug => $plugin ) {
				if ( 21113424 === $plugin['id'] && 'elegant-elements-fusion-builder' === $args->slug ) {
					...
					$response->author          = sprintf( '<a href="%s" target="_blank">%s</a>', $plugin['author_url'], $plugin['author'] );
					$response->homepage        = $plugin['url'];
					$response->requires        = $plugin['requires'];
					$response->tested          = $wp_version;
					$response->active_installs = $plugin['number_of_sales'];
					$response->last_updated    = $plugin['updated_at'];
					$response->sections        = array(
						'description' => __( '<strong>Elegant Elements for Fusion Builder</strong> plugin is an add-on for Fusion Builder page builder for Avada. A must-have add-on for Avada and Fusion Builder. Design your stunning website, even more, faster with our ever-growing library of 50+ elements and 100+ custom-designed and unique templates.', 'elegant-elements' ),
						'changelog'   => sprintf( __( 'To get the latest changelog, please <a href="%s" target="_blank">Click Here</a>', 'elegant-elements' ), 'https://fusionelegantelements.com/changelog/' ),
					);
					$response->banners['low']  = $plugin['landscape_url'];
					$response->rating          = $plugin['rating']['average'];
