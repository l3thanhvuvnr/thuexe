...
class Elegant_Envato_API {
	...
	public function __clone() {
		_doing_it_wrong( __FUNCTION__, esc_html__( 'Cheatin&#8217; huh?', 'elegant-elements' ), '1.0.0' );
	}
	...
	public function request( $url, $args = array() ) {
		...
		if ( empty( $this->token ) ) {
			return new WP_Error( 'api_token_error', __( 'An API token is required.', 'elegant-elements' ) );
		}
		...
		if ( 403 === $response_code ) {
			...
			return new WP_Error( $response_code, __( 'Expired access token. Please try again.', 'elegant-elements' ) );
		}
		...
		if ( 200 !== $response_code ) {
			return new WP_Error( $response_code, __( 'An unknown API error occurred.', 'elegant-elements' ) );
		}

		$return = json_decode( wp_remote_retrieve_body( $response ), true );
		if ( null === $return ) {
			return new WP_Error( 'api_error', __( 'An unknown API error occurred.', 'elegant-elements' ) );
		}
	...
	public function plugins( $args = array(), $page = '' ) {
		$plugins = get_transient( 'elegant_envato_buyer_plugins' );
		...
		if ( false === $plugins ) {
			...
			set_transient( 'elegant_envato_buyer_plugins', $plugins, 60 * 60 );
		}
